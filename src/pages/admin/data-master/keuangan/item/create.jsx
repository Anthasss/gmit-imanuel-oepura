...
const itemKeuanganService = {
  ...
  getByKategori: async (kategoriId) => {
    const response = await axios.get(
      `/api/keuangan/item?kategoriId=${kategoriId}`
    );
    return response.data;
  },
};
...
export default function CreateItemKeuanganPage() {
  ...
  // Query untuk existing items berdasarkan kategori
  const { data: existingItems, refetch: refetchItems } = useQuery({
    queryKey: ["existing-items", selectedKategori],
    queryFn: () => itemKeuanganService.getByKategori(selectedKategori),
    enabled: !!selectedKategori,
  });

  // Initialize dengan item level 1 jika belum ada
  useEffect(() => {
    if (selectedKategori && existingItems?.data?.items) {
  ...
  }, [selectedKategori, existingItems]);
  ...
  const saveItems = async () => {
    ...
    try {
      ...
      router.push("/admin/master-data/item-keuangan");
    } catch (error) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={saveItems}
          disabled={saving || !selectedKategori}
          className="min-w-32"
        >
